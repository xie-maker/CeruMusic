...
onMounted(() => {
  ...
  // 监听 Logto 回调
  getPlatformService().music.on('logto-callback', (_: any, url: string) => {
  ...
  // 使用 PlatformService 的 onMusicCtrl 监听音乐控制事件
  getPlatformService().onMusicCtrl((_event: Event, args: any) => {
    const name = args?.name || args?.type
    const val = args?.val
    if (name) forward(name, val)
  })

  // Audio Output Selector Shortcut
  // Remove existing listeners to prevent duplicates/HMR issues
  getPlatformService().music.removeAllListeners('hotkeys:toggle-audio-output-selector')
  getPlatformService().music.on('hotkeys:toggle-audio-output-selector', () => {
  ...
  // 全局监听打开歌单文件
  getPlatformService().music.on('open-playlist-file', (_: any, filePath: string) => {
  ...
  getPlatformService().music.invoke('get-pending-open-playlist-files')
    .then((paths: string[]) => {
      paths?.forEach((p) => {
  ...
  router.isReady().then(() => {
    setTimeout(() => {
      ...
      getPlatformService().music.invoke('get-pending-lt-codes')
        .then((codes: string[]) => {
  ...
  getPlatformService().music.on('lt-share-open', (_: any, payload: { code: string }) => {
